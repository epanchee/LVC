...
const double volt_init=-84.5; /// steady-state potential
const double volt_stim=0;
double
    I_stim, t_stim, tipval=-40;

double c_gNa=1, /// parameters for diminishing conductivities for INa, Isi, IK
        c_gsi=1,
        c_gK=1;

double CONSTANTS[24];

/// we can choose between Luo--Rudy 1991 model (problems with spiral wave initiation) and ten Tusscher--Panfilov 2003 variant of LR model
// #define LR91
#define LR_TP03
/*
...
void initConsts(double* CONSTANTS)
{
    CONSTANTS[0] = 8314;
    CONSTANTS[1] = 310;
    CONSTANTS[2] = 96484.6;
...
#ifdef LR91
    CONSTANTS[19] =  c_gK*0.282000* pow((CONSTANTS[13]/5.40000), 1.0 / 2);
#endif
#ifdef LR_TP03
    CONSTANTS[19] =  c_gK*0.705* pow((CONSTANTS[13]/5.40000), 1.0 / 2);
#endif
...
#ifdef LR91
    ALGEBRAIC[15] =  c_gsi*0.0900000*STATES[5]*STATES[6]*(STATES[0] - ALGEBRAIC[14]);
#endif
#ifdef LR_TP03
    ALGEBRAIC[15] =  c_gsi*0.045*STATES[5]*STATES[6]*(STATES[0] - ALGEBRAIC[14]);
#endif
...
{
    double RATES[8];
    /// VOI=-1 (no stimulation by model itself, only stimuli from main())
    computeRates(-1, CONSTANTS, RATES, uv0);
    int i;

    for (i=0; i<Nvar+1; i++)
        uv1[i] = uv0[i] + dt*RATES[i];
    uv1[indV] += dt*(DivDGradU+Istim);
}
